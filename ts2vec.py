...
class SpatioTemporalTS2Vec(pl.LightningModule):
    '''The TS2Vec model'''
    ...
    def training_step(self, batch, batch_idx):
        ...
        # input for self._net: [B, T1, R, H, W]
        out1 = self._net(take_per_row(x, crop_offset + crop_eleft, crop_right - crop_eleft))
        out1 = out1[:, -crop_l:]
        # out1: [R, T_after, Co]
        # Co - channel output from encoder
        
        # input for self._net: [B, T2, R, H, W]
        out2 = self._net(take_per_row(x, crop_offset + crop_left, crop_eright - crop_left))
        out2 = out2[:, :crop_l]
        # out2: [R, T_after, Co]
        # Co - channel output from encoder

        train_loss = self.criterion(out1, out2)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        val_loss = self.criterion(out1, out2)
